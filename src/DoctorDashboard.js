...
function DoctorDashboard() {
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            await axios.post(`${API_URL}/api/availability/set`, {
                date: date,
                slots: slotArray
            });

            setSuccess(`Availability updated successfully for ${new Date(date + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}!`);
            setDate("");
            setSlots("");
    ...
    const fetchBookings = async () => {
        try {
            const res = await axios.get(`${API_URL}/api/booking/all`);
    ...
    const fetchAllAvailability = async () => {
        try {
            const res = await axios.get(`${API_URL}/api/availability/all`);
            // Filter out empty slots and past dates
            const data = res.data.filter(d => d.slots && d.slots.length > 0 && d.date >= today);
                        ...
                        <div className="card space-y-6 border-t-4 border-t-[#588157]">
                            <div>
                                <h3 className="text-lg font-bold text-[#344e41] flex items-center gap-2">
                                    ...
                                    Override Slots (Optional)
                                </h3>
                                <p className="text-xs text-[#6b705c] mt-2">
                                    Patients can already book default slots automatically
                                    (16:00–20:00 on weekdays, 10:00–13:45 on Sundays) without you doing anything here.
                                    Use this form only if a specific date needs different timings.
                                </p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    ...
                                    {date && (
                                        <p className="text-xs text-[#588157] mt-2 font-medium">
                                            Overriding: {new Date(date + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}
                                        </p>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        required
                                    />
                                    <p className="text-xs text-[#7a8b76] dark:text-[#6b705c] mt-2">Comma separated list of available times. This replaces the default timing for this date only.</p>
                                </div>
                                ...
                                >
                                    {saving ? "Saving..." : "Save Override for This Date"}
                                </button>
                            </form>
                        </div>
                            ...
                            {allAvailability.length > 0 ? (
                                <div className="space-y-4">
                                    {displayedAvailability.map((item, index) => {
                                        ...
                                        const isDate = item.date.includes("-");
                                        ...
                                        return (
                                            <div key={index} className="p-4 border border-[#d8e2c8] rounded-xl bg-[#f8f8f2] hover:border-[#a3b18a] transition-colors">
                                                <h4 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-3 flex items-center justify-between">
                                                    {item.date === today ? <span className="text-[#588157]">Today</span> : displayDate}
                                ...
                                <div className="text-center py-10 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-700/50">
                                    ...
                                    <p className="text-sm text-[#6b705c] font-medium">No date-specific overrides — all upcoming dates use default timings.</p>
                                </div>
                            )}
                        </div>
                    </div>
